...
class Register extends Component
{
    ...
    public function registerRole()
    {
        ...
        try{
            DB::beginTransaction();
            $isRegistered = (new RoleRepository())->storeRole([
                'name' => $this->name,
                'description' => $this->description,
                'level' => $this->level,
                'permissions' => array_pluck($this->selectedPermissions, "id")
            ]);
            if($isRegistered){
                DB::commit();
                $this->reset();
                ...
                $this->selectedOption = '';
                $this->dispatch("flash-alert", type: "success", title: "Success", message: "Role is successfully registered!.");
            }else{
                DB::rollBack();
                $this->dispatch("flash-alert", type: "error", title: "Error", message: "Unable to register Role!.");
            }
        }catch(Exception $e){
            DB::rollBack();
